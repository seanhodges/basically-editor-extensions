...
- [ ] 3.2 On a refusal that named candidates, offer them and ask again with the
      same bytes and the machine named.
- [ ] 3.3 Open the source as an untitled listing with its language set, and show
      it.
- [ ] 3.4 Put the warnings to the user and into the shared output channel.
- [ ] 3.5 Where blocks came back: say what they are, ask for a folder, write each
      one there, and name what was written. Declining leaves the listing open and
      is not a failure.
- [ ] 3.6 Register it in `extension.ts` beside the others.

## 4. Offer it twice

- [ ] 4.1 Contribute the command in `clients/vscode/package.json` with a codicon
      icon, its palette entry carrying no `when` clause since it needs no
      listing.
- [ ] 4.2 Add the third `editor/title` navigation entry, after export, bound to a
      BASIC listing like its neighbours.
- [ ] 4.3 Move `basically.server.version` to the release carrying
...
- [ ] 5.1 Extend `clients/vscode/test/programTransfer.test.mjs`: an import
...
- [ ] 5.2 Extend the manifest cross-check with the third command and its menu
      entry, and with the fact that the import command's palette entry is
      unconditional while its toolbar entry is not.
- [ ] 5.3 Extend the case in `clients/vscode/test/handshake.test.mjs` that the
...
- [ ] 5.4 Run that suite against a toolchain checkout carrying the paired change:
      `BASICALLY_SERVER_PATH=/path/to/basically/scripts/basically npm test`.

## 6. Say what it does

- [ ] 6.1 Extend the README section the export change added: what reading a file
...
- [ ] 7.1 `npm run lint`
- [ ] 7.2 `npm run build`
- [ ] 7.3 `npm run server` — needs npm registry access; where it is unavailable,
      say so rather than reporting a green run.
- [ ] 7.4 `npm test`
- [ ] 7.5 `npm run package` — required: the manifest gains a command and a menu
      entry.
