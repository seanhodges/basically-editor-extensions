...
- [ ] 1.1 Add `BuiltFile` and `BuildOutcome` to `clients/vscode/src/operations.ts`,
      naming only what this client asks for, in the voice of the report types
      already there.
- [ ] 1.2 Add the `build` wrapper beside `run` and `info`, taking the machine, the
      source, the name the first file is to be written under, and the chosen
      target.
- [ ] 1.3 Name `buildTargets` on `MachineFacts`. The server already reports them
      under `info`; this client has never asked for them.

## 2. Decide what is to be done, apart from the editor

- [ ] 2.1 Add `clients/vscode/src/programTransfer.ts`, importing no `vscode`, in
      the manner of `machineStatus.ts`.
- [ ] 2.2 Plan an export from the machines the server has, the listing's own
...
- [ ] 2.3 Turn a `BuildOutcome` into what the user is told: a fatal problem means
      nothing was written and why; otherwise every file and where it went.

## 3. Do the editor's part

- [ ] 3.1 Add `clients/vscode/src/programTransferCommands.ts`: the command, and
...
- [ ] 3.2 The command: settle the machine, ask `info` for the machine's targets,
      offer them, seed a save dialog from the listing's name and the target's
      extension, and call `build` with the buffer's text.
- [ ] 3.3 Write the files: the first where the user said, the rest beside it
      under their own names, and say what was written where.
- [ ] 3.4 Register it in `extension.ts` alongside the existing three, and dispose
      the conversation on deactivate.

## 4. Offer it twice

- [ ] 4.1 Contribute the command in `clients/vscode/package.json`, category
...
- [ ] 4.2 Give it and the existing run command a codicon icon.
- [ ] 4.3 Add the `editor/title` navigation entries — run, then export — both
      bound to a BASIC listing. This is the manifest's first toolbar.

## 5. Check it

- [ ] 5.1 Add `clients/vscode/test/programTransfer.test.mjs`, client-against-itself
...
- [ ] 5.2 In the same suite, cross-check the manifest against itself as
      ...
      that can draw one.
- [ ] 5.3 Extend `clients/vscode/test/handshake.test.mjs`, inside the block that
...
- [ ] 6.1 Add a section to `clients/vscode/README.md` covering the command, the
...
- [ ] 7.2 `npm run lint`
- [ ] 7.3 `npm run build`
- [ ] 7.4 `npm run server` — needs npm registry access; where it is unavailable,
      say so rather than reporting a green run.
- [ ] 7.5 `npm test`
- [ ] 7.6 `npm run package` — required: the manifest gains a command and a menu.
